...
progress <- function(n) {
  ...
  cat(sprintf("\rTask %d/%d completed", n, np))  # 顯示當前進度
}
...
CAT_data <- foreach(i = 1:np, .combine = rbind, .packages = "catR", .options.snow = opts) %dopar% {
  ...
  for (j in 1:TL) {
    ...
    admin.item <- which.max(infosum_20point) # 找出訊息量最大值(返回下標)
...
cat("MFII模擬時間：", exec_timeCAT_MFII, "\n")
...
cat("MLWI模擬時間：", exec_timeCAT_MLWI, "\n")
